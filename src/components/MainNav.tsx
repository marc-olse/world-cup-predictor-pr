  ...
  return (
    <nav className="flex flex-wrap items-center gap-1 text-xs sm:gap-2">
      {items.map((item) => {
        ...
        return (
          <Link
            className={`grid min-h-14 min-w-16 justify-items-center gap-1 rounded-md px-2 py-2 text-center font-semibold leading-tight transition sm:min-w-20 ${
              active
          ...
          >
            {item.icon}
            <span className="max-w-20 text-[0.64rem] sm:text-[0.68rem]">
              {item.label}
            </span>
          </Link>
        );
